...
def lesson3_concept_1(matrix_size: int = 1024) -> None:
    ...
    partition_over_rows = jax.sharding.PartitionSpec("myaxis")
    ...
    partition_over_columns = jax.sharding.PartitionSpec(None,"myaxis")
...
def lesson3_concept_3(
    ...
    mesh_axes: list = ["x", "y"],
    partition_spec_A: tuple = ("x", None),
    partition_spec_B: tuple = (None, "y"),
...
if args.l3_p2:
    ...
    lesson3_concept_2(
        ...
        mesh_shape=(2,2),
        mesh_axes=["x", "y"],
        partition_spec=("x", "y"),  # Split both dimensions
        name="2D Grid Partitioning: Each device gets 512x512 quadrant"
        # Mesh shape: 2 x 2 (4 devices arranged in a 2x2 grid)
        # Row dimension: 1024 ÷ 2 = 512 (split across "x" axis)
        # Column dimension: 1024 ÷ 2 = 512 (split across "y" axis)
    ...
    lesson3_concept_2(
        ...
        mesh_shape=(2,2),
        mesh_axes=["x", "y"],
        partition_spec=("y", "x"),  # Split both dimensions with reversed mapping
        name="2D Grid Partitioning (Reversed): 512x512 with different device assignment"
        # Mesh shape: 2 x 2 (4 devices arranged in a 2x2 grid)
        # Row dimension: 1024 ÷ 2 = 512 (split across "y" axis)
        # Column dimension: 1024 ÷ 2 = 512 (split across "x" axis)
    ...
    lesson3_concept_2(
        ...
        mesh_shape=(2,2),
        mesh_axes=["x", "y"],
        partition_spec=(("x", "y"), None),  # Combine axes for first dimension only
        name="Vertical Strip Partitioning: Four 256x1024 vertical strips"
        # Mesh shape: 2 x 2 (4 devices arranged in a 2x2 grid)
        # Row dimension: 1024 ÷ (2×2) = 256 (split across combined "x,y" axes)
        # Column dimension: 1024 (not split - None in spec)
    ...
    lesson3_concept_2(
        ...
        mesh_shape=(2,2),
        mesh_axes=["x", "y"],
        partition_spec=(("y", "x"), ),  # Different axis combination order
        name="Vertical Strip Partitioning (Reordered): Four 256x1024 strips, different order"
        # Mesh shape: 2 x 2 (4 devices arranged in a 2x2 grid)
        # Row dimension: 1024 ÷ (2×2) = 256 (split across combined "y,x" axes)
    ...
    lesson3_concept_2(
        ...
        mesh_shape=(2,2),
        mesh_axes=["x", "y"],
        partition_spec=(None, ("x")),  # Split only columns
        name="Horizontal Strip Partitioning: Two 1024x512 horizontal strips"
        # Mesh shape: 2 x 2 (4 devices arranged in a 2x2 grid)
        # Row dimension: 1024 (not split - None in spec)
        # Column dimension: 1024 ÷ 2 = 512 (split across x axis only)
    ...
    lesson3_concept_2(
        ...
        mesh_shape=(1,4),
        mesh_axes=["x", "y"],
        partition_spec=(None, ("x")),  # Attempt to split, but mesh shape prevents it
        name="Full Replication (1x4 mesh): Mesh shape forces full replication"
        # Mesh shape: 1 x 4 (4 devices arranged in a 1x4 grid)
        # Row dimension: 1024 (not split - None in spec)
        # Column dimension: 1024 (not split - x axis has length 1)
        # Result: Full replication because x dimension has size 1
    )
...
if args.l3_p3:
    ...
    lesson3_concept_3(
        ...
        mesh_shape=(2,2),
        mesh_axes=["x", "y"],
        partition_spec_A=("x", None),  # Split rows
        partition_spec_B=(None, "y"),  # Split columns
